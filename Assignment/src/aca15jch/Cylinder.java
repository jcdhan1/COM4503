...
/**
 * @author aca15jch
 * Some code reused from Dr. Maddock's Sphere class.
 */
public final class Cylinder {
	...
	private static int[] createIndices() {
		...
		for (int j = 2; j < YLAT + 1; ++j) {
			for (int i = 0; i < XLONG + 1; ++i) {
				int base = j * (XLONG + 1) * 6;
				indices[base + i * 6 + 0] = j * XLONG + 2 + i;
				indices[base + i * 6 + 1] = j * XLONG + 2 + i + 1;
				indices[base + i * 6 + 2] = (j + 1) * XLONG + 2 + i + 1;
				indices[base + i * 6 + 3] = j * XLONG + 2 + i;
				indices[base + i * 6 + 4] = (j + 1) * XLONG + 2 + i + 1;
				indices[base + i * 6 + 5] = (j + 1) * XLONG + 2 + i;
			}
		}
		return indices;
	}
