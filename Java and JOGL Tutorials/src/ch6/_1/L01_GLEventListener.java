...
public class L01_GLEventListener implements GLEventListener {
    ...
    public float oscillate(double elapsedTime, double f, double a, boolean cos) {
        return (float) (a * Math.cos(2 * Math.PI * f * elapsedTime));
    }
    ...
    private Mat4 getLightModelMatrix() {
        double elapsedTime = getSeconds() - startTime;
        lightPosition.x = 5.0f * oscillate(elapsedTime,0.1, 1); //(float) (Math.sin(Math.toRadians(elapsedTime * 50)));
        lightPosition.y = 0;//3.0f;
        lightPosition.z = 5.0f * oscillate(elapsedTime,0.1, 1, true); //(float) (Math.sin(Math.toRadians(elapsedTime * 50)));
        Mat4 modelMatrix = new Mat4(1);
        modelMatrix = Mat4.multiply(Mat4Transform.scale(0.3f, 0.3f, 0.3f), modelMatrix);
        modelMatrix = Mat4.multiply(Mat4Transform.translate(lightPosition), modelMatrix);
        return modelMatrix;
